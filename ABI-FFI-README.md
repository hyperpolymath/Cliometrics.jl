...
# Cliometrics ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libcliometrics.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── cliometrics.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── cliometrics.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/cliometrics.h
```
...
```c
#include "cliometrics.h"

int main() {
    void* handle = cliometrics_init();
    if (!handle) return 1;

    int result = cliometrics_process(handle, 42);
    if (result != 0) {
        const char* err = cliometrics_last_error();
...
Compile with:
```bash
gcc -o example example.c -lcliometrics -L./zig-out/lib
```

### From Idris2

```idris
import Cliometrics.ABI.Foreign
...
```rust
#[link(name = "cliometrics")]
extern "C" {
    fn cliometrics_init() -> *mut std::ffi::c_void;
    fn cliometrics_free(handle: *mut std::ffi::c_void);
    fn cliometrics_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = cliometrics_init();
        assert!(!handle.is_null());

        let result = cliometrics_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcliometrics = "libcliometrics"

function init()
    handle = ccall((:cliometrics_init, libcliometrics), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:cliometrics_process, libcliometrics), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:cliometrics_free, libcliometrics), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/cliometrics.h
   ```
